Config = {}

Config.Doors = {
	
	--[[
	
	{
		objName = 'vesp_door1',
		objCoords  = {x = -1093.35, y = -845.04, z = 23.19},
		textCoords = {x = -1093.35, y = -845.04, z = 23.19}, -------------------
		authorizedJobs = { 'police', 'emspolice' },
		locked = true,
		distance = 2.5
	},

	{
		objName = 'vesp_door1',
		objCoords  = {x = -1094.91, y = -842.99, z = 23.18},
		textCoords = {x = -1094.91, y = -842.99, z = 23.18}, -----------------------
		authorizedJobs = { 'police', 'emspolice' },
		locked = true,
		distance = 2.5
	},

	{
		objName = 'v_ilev_rc_door2',
		objCoords  = {x = -1085.67, y = -832.38, z = 15.03},
		textCoords = {x = -1085.67, y = -832.38, z = 15.03},
		authorizedJobs = { 'police', 'emspolice' },
		locked = true,
		distance = 2.5
	},
	
	{
		objName = 'v_ilev_rc_door2',
		objCoords  = {x = -1087.24, y = -830.32, z = 15.03},
		textCoords = {x = -1087.24, y = -830.32, z = 15.03},
		authorizedJobs = { 'police', 'emspolice' },
		locked = true,
		distance = 2.5
	},
	
	{
		objName = 'v_ilev_rc_door2',
		objCoords  = {x = -1076.40, y = -824.14, z = 15.03},
		textCoords = {x = -1076.40, y = -824.14, z = 15.03},
		authorizedJobs = { 'police', 'emspolice' },
		locked = true,
		distance = 2.5
	},
	
	{
		objName = 'v_ilev_rc_door2',
		objCoords  = {x = -1074.33, y = -822.57, z = 15.03},
		textCoords = {x = -1074.33, y = -822.57, z = 15.03},
		authorizedJobs = { 'police', 'emspolice' },
		locked = true,
		distance = 2.5
	},
	
	{
		objName = 'vesp_door1',
		objCoords  = {x = -1079.62, y = -826.25, z = 11.18},
		textCoords = {x = -1079.62, y = -826.25, z = 11.18},
		authorizedJobs = { 'police', 'emspolice' },
		locked = true,
		distance = 2.5
	},
	
	{
		objName = 'vesp_door1',
		objCoords  = {x = -1077.57, y = -824.69, z = 11.18},
		textCoords = {x = -1077.57, y = -824.69, z = 11.18},
		authorizedJobs = { 'police', 'emspolice' },
		locked = true,
		distance = 2.5
	},
	
	{
		objName = 'vesp_door1',
		objCoords  = {x = -1085.72, y = -832.41, z = 11.18},
		textCoords = {x = -1085.72, y = -832.41, z = 11.18},
		authorizedJobs = { 'police', 'emspolice' },
		locked = true,
		distance = 2.5

			{
		objName = 'vesp_door1',
		objCoords  = {x = -1093.35, y = -845.04, z = 23.19},
		textCoords = {x = -1093.35, y = -845.04, z = 23.19},
		authorizedJobs = { 'police', 'emspolice' },
		locked = true,
		distance = 2.5
	},

	{
		objName = 'vesp_door1',
		objCoords  = {x = -1094.91, y = -842.99, z = 23.18},
		textCoords = {x = -1094.91, y = -842.99, z = 23.18},
		authorizedJobs = { 'police', 'emspolice' },
		locked = true,
		distance = 2.5
	},
	},]]--vesp_door1

	{
		textCoords = vector3(-1094.16, -844.04, 27.79),
		authorizedJobs = { 'police', 'emspolice' },
		locking = false,
		locked = true,
		pickable = false,
		distance = 2.5,
		doors = {
			{
				objName = 'vesp_door1',
				objYaw = 128.0,
				objCoords = vector3(-1094.91, -842.99, 26.79)
			},

			{ 
				objName = 'vesp_door1',
				objYaw = -53.5,
				objCoords = vector3(-1093.35, -845.04, 26.79)
			}
		}
	},

	{
		textCoords = vector3(-1086.5, -831.38, 12.00),
		authorizedJobs = { 'police', 'emspolice' },
		locking = false,
		locked = true,
		pickable = false,
		distance = 2.5,
		doors = {
			{
				objName = 'vesp_door1',
				objYaw = -53.0,
				objCoords = vector3(-1086.15, -831.91, 10.73)
			},

			{ 
				objName = 'vesp_door1',
				objYaw = 128.0,
				objCoords = vector3(-1086.91, -830.86, 10.73)
			}
		}
	},

	{
		textCoords = vector3(-1086.5, -831.38, 15.72),
		authorizedJobs = { 'police', 'emspolice' },
		locking = false,
		locked = true,
		pickable = false,
		distance = 2.5,
		doors = {
			{
				objName = 'v_ilev_rc_door2',
				objYaw = 128.0,
				objCoords = vector3(-1086.15, -831.91, 14.72)
			},

			{ 
				objName = 'v_ilev_rc_door2',
				objYaw = -53.0,
				objCoords = vector3(-1086.91, -830.86, 14.72)
			}
		}
	},

	{
		textCoords = vector3(-1078.59, -825.47, 12.00),
		authorizedJobs = { 'police', 'emspolice' },
		locking = false,
		locked = true,
		pickable = false,
		distance = 2.5,
		doors = {
			{
				objName = 'vesp_door1',
				objYaw = -143.7,
				objCoords = vector3(-1079.22, -825.92, 10.73)
			},

			{ 
				objName = 'vesp_door1',
				objYaw = 38.0,
				objCoords = vector3(-1078.59, -825.47, 10.73)
			}
		}
	},

	{
		textCoords = vector3(-1075.38, -823.36, 15.60),
		authorizedJobs = { 'police', 'emspolice' },
		locking = false,
		locked = true,
		pickable = false,
		distance = 2.5,
		doors = {
			{
				objName = 'v_ilev_rc_door2',
				objYaw = -143.7,
				objCoords = vector3(-1075.33, -822.57, 14.60)
			},

			{ 
				objName = 'v_ilev_rc_door2',
				objYaw = 38.0,
				objCoords = vector3(-1075.76, -822.86, 14.60)
			}
		}
	},

	{
		textCoords = vector3(-1081.43, -821.82, 12.00),
		authorizedJobs = { 'police', 'emspolice' },
		locking = false,
		locked = true,
		pickable = false,
		distance = 2.5,
		doors = {
			{
				objName = 'vesp_door1',
				objYaw = -143.7,
				objCoords = vector3(-1081.82, -822.12, 10.73)
			},

			{ 
				objName = 'vesp_door1',
				objYaw = 38.0,
				objCoords = vector3(-1080.96, -821.45, 10.73)
			}
		}
	},

	{
		textCoords = vector3(-1094.44, -835.01, 11.00),
		authorizedJobs = { 'police', 'emspolice' },
		locking = false,
		locked = true,
		pickable = false,
		distance = 2.5,
		doors = {
			{
				objName = 'vesp_door1',
				objYaw = 38.0,
				objCoords = vector3(-1093.85, -834.56, 9.72)
			},

			{ 
				objName = 'vesp_door1',
				objYaw = -143.7,
				objCoords = vector3(-1095.01, -835.42, 9.72)
			}
		}
	},

	{
		textCoords = vector3(-1094.16, -844.04, 35.36),
		authorizedJobs = { 'police', 'emspolice' },
		locking = false,
		locked = true,
		pickable = false,
		distance = 2.5,
		doors = {
			{
				objName = 'vesp_door1',
				objYaw = 128.0,
				objCoords = vector3(-1094.91, -842.99, 34.36)
			},

			{ 
				objName = 'vesp_door1',
				objYaw = -53.5,
				objCoords = vector3(-1093.35, -845.04, 34.36)
			}
		}
	},

	{
		textCoords = vector3(-1094.16, -844.04, 31.76),
		authorizedJobs = { 'police', 'emspolice' },
		locking = false,
		locked = true,
		pickable = false,
		distance = 2.5,
		doors = {
			{
				objName = 'vesp_door1',
				objYaw = 128.0,
				objCoords = vector3(-1094.91, -842.99, 30.76)
			},

			{ 
				objName = 'vesp_door1',
				objYaw = -53.5,
				objCoords = vector3(-1093.35, -845.04, 30.76)
			}
		}
	},

	{
		textCoords = vector3(-1094.16, -844.04, 23.77),
		authorizedJobs = { 'police', 'emspolice' },
		locking = false,
		locked = true,
		pickable = false,
		distance = 2.5,
		doors = {
			{
				objName = 'vesp_door1',
				objYaw = 128.0,
				objCoords = vector3(-1094.91, -842.99, 23.18)
			},

			{ 
				objName = 'vesp_door1',
				objYaw = -53.5,
				objCoords = vector3(-1093.35, -845.04, 23.19)
			}
		}
	},

	{
		objName = 'vesp_dopr',
		objYaw = -142.5,
		objCoords  = vector3(-1090.02, -841.02, 37.2),
		textCoords = vector3(-1090.02, -841.02, 37.2),
		authorizedJobs = { 'police', 'emspolice' },
		locking = false,
		locked = true,
		pickable = false,
		distance = 1.5,
	},

	{
		textCoords = vector3(-1112.02, -847.93, 14.40),
		authorizedJobs = { 'police', 'emspolice' },
		locking = false,
		locked = true,
		pickable = false,
		distance = 2.5,
		doors = {
			{
				objName = 'vesp_glav_door',
				objYaw = 128.0,
				objCoords = vector3(-1112.38, -847.44, 12.85)
			},

			{ 
				objName = 'vesp_glav_door',
				objYaw = -53.5,
				objCoords = vector3(-1111.66, -848.40, 12.85)
			}
		}
	},

	{
		textCoords = vector3(-1069.35, -824.25, 28.19),
		authorizedJobs = { 'police', 'emspolice' },
		locking = false,
		locked = true,
		pickable = false,
		distance = 2.5,
		doors = {
			{
				objName = 'vesp_door1',
				objYaw = 173.0,
				objCoords = vector3(-1070.63, -824.09, 27.19)
			},

			{ 
				objName = 'vesp_door1',
				objYaw = -8.0,
				objCoords = vector3(-1068.07, -824.42, 27.19)
			}
		}
	},

	{
		objName = 'v_ilev_ph_cellgate',
		objYaw = -142.5,
		objCoords  = vector3(-1072.98, -826.92, 6.48),
		textCoords = vector3(-1072.98, -826.92, 6.48),
		authorizedJobs = { 'police', 'emspolice' },
		locking = false,
		locked = true,
		pickable = false,
		distance = 1.5,
	},

	{
		objName = 'v_ilev_arm_secdoor',
		objYaw = 30.0,
		objCoords  = vector3(1855.66, 3687.96, 31.10),
		textCoords = vector3(1855.66, 3687.96, 31.10),
		authorizedJobs = { 'police', 'emspolice' },
		locking = false,
		locked = true,
		pickable = false,
		distance = 1.5,
	},

	{
		objName = 'v_ilev_arm_secdoor',
		objYaw = 30.0,
		objCoords  = vector3(1852.42, 3686.13, 31.10),
		textCoords = vector3(1852.42, 3686.13, 31.10),
		authorizedJobs = { 'police', 'emspolice' },
		locking = false,
		locked = true,
		pickable = false,
		distance = 1.5,
	},

	{
		objName = 'v_ilev_ph_cellgate',
		objYaw = -52.0,
		objCoords  = vector3(-1086.3, -827.69, 6.48),
		textCoords = vector3(-1086.3, -827.69, 6.48),
		authorizedJobs = { 'police', 'emspolice' },
		locking = false,
		locked = true,
		pickable = false,
		distance = 1.5,
	},

	{
		objName = 'v_ilev_ph_cellgate',
		objYaw = -52.0,
		objCoords  = vector3(-1089.33, -829.69, 6.48),
		textCoords = vector3(-1089.33, -829.69, 6.48),
		authorizedJobs = { 'police', 'emspolice' },
		locking = false,
		locked = true,
		pickable = false,
		distance = 1.5,
	},

	{
		objName = 'v_ilev_ph_cellgate',
		objYaw = -52.0,
		objCoords  = vector3(-1091.75, -826.35, 6.48),
		textCoords = vector3(-1091.75, -826.35, 6.48),
		authorizedJobs = { 'police', 'emspolice' },
		locking = false,
		locked = true,
		pickable = false,
		distance = 1.5,
	},

	{
		objName = 'v_ilev_ph_cellgate',
		objYaw = -52.0,
		objCoords  = vector3(-1088.67, -824.09, 6.48),
		textCoords = vector3(-1088.67, -824.09, 6.48),
		authorizedJobs = { 'police', 'emspolice' },
		locking = false,
		locked = true,
		pickable = false,
		distance = 1.5,
	},

	{
		objName = 'v_ilev_ph_cellgate',
		objYaw = -52.0,
		objCoords  = vector3(-1091.2, -820.97, 6.48),
		textCoords = vector3(-1091.2, -820.97, 6.48),
		authorizedJobs = { 'police', 'emspolice' },
		locking = false,
		locked = true,
		pickable = false,
		distance = 1.5,
	},

	{
		objName = 'v_ilev_ph_cellgate',
		objYaw = -52.0,
		objCoords  = vector3(-1091.2, -820.97, 6.48),
		textCoords = vector3(-1091.2, -820.97, 6.48),
		authorizedJobs = { 'police', 'emspolice' },
		locking = false,
		locked = true,
		pickable = false,
		distance = 1.5,
	},

	{
		objName = 'v_ilev_ph_cellgate',
		objYaw = -52.0,
		objCoords  = vector3(-1094.11, -823.18, 6.48),
		textCoords = vector3(-1094.11, -823.18, 6.48),
		authorizedJobs = { 'police', 'emspolice' },
		locking = false,
		locked = true,
		pickable = false,
		distance = 1.5,
	},

	{
		objName = 'v_ilev_ph_cellgate',
		objYaw = -52.0,
		objCoords  = vector3(-1096.44, -820.11, 6.48),
		textCoords = vector3(-1096.44, -820.11, 6.48),
		authorizedJobs = { 'police', 'emspolice' },
		locking = false,
		locked = true,
		pickable = false,
		distance = 1.5,
	},

	{
		objName = 'v_ilev_ph_cellgate',
		objYaw = -142.5,
		objCoords  = vector3(-1087.04, -829.39, 5.48),
		textCoords = vector3(-1087.04, -829.39, 5.48),
		authorizedJobs = { 'police', 'emspolice' },
		locking = false,
		locked = true,
		pickable = false,
		distance = 1.5,
	},

	{
		objName = 'v_ilev_rc_door2',
		objYaw = -54.0,
		objCoords  = vector3(-1078.04, -814.11, 6.48),
		textCoords = vector3(-1078.04, -814.11, 5.48),
		authorizedJobs = { 'police', 'emspolice' },
		locking = false,
		locked = true,
		pickable = false,
		distance = 1.5,
	},

	{
		objName = 'v_ilev_rc_door2',
		objYaw = -54.0,
		objCoords  = vector3(-1081.78, -816.79, 6.48),
		textCoords = vector3(-1081.78, -816.79, 5.48),
		authorizedJobs = { 'police', 'emspolice' },
		locking = false,
		locked = true,
		pickable = false,
		distance = 1.5,
	},

	{
		textCoords = vector3(-1074.54, -822.41, 6.48),
		authorizedJobs = { 'police', 'emspolice' },
		locking = false,
		locked = true,
		pickable = false,
		distance = 2.5,
		doors = {
			{
				objName = 'v_ilev_rc_door2',
				objYaw = 37.0,
				objCoords = vector3(-1075.08, -822.82, 5.48)
			},

			{ 
				objName = 'v_ilev_rc_door2',
				objYaw = -142.5,
				objCoords = vector3(-1074.07, -822.02, 5.48)
			}
		}
	},

	{
		textCoords = vector3(-1091.51, -834.99, 6.48),
		authorizedJobs = { 'police', 'emspolice' },
		locking = false,
		locked = true,
		pickable = false,
		distance = 2.5,
		doors = {
			{
				objName = 'v_ilev_rc_door2',
				objYaw = 127.0,
				objCoords = vector3(-1091.19, -835.52, 5.48)
			},

			{
				objName = 'v_ilev_rc_door2',
				objYaw = -52.0,
				objCoords = vector3(-1091.98, -834.41, 5.48)
			}
		}
	},

	{
		textCoords = vector3(-1090.27, -847.72, 5.88),
		authorizedJobs = { 'police', 'emspolice' },
		locking = false,
		locked = true,
		pickable = false,
		distance = 2.5,
		doors = {
			{
				objName = 'v_ilev_rc_door2',
				objYaw = -53.0,
				objCoords = vector3(-1090.67, -847.21, 4.88)
			},

			{
				objName = 'v_ilev_rc_door2',
				objYaw = 127.8,
				objCoords = vector3(-1089.98, -848.16, 4.88)
			}
		}
	},

	{
		textCoords = vector3(-1057.79, -839.91, 6.14),
		authorizedJobs = { 'police', 'emspolice' },
		locking = false,
		locked = true,
		pickable = false,
		distance = 2.5,
		doors = {
			{
				objName = 'v_ilev_rc_door2',
				objYaw = 37.5,
				objCoords = vector3(-1058.25, -840.17, 5.14)
			},

			{
				objName = 'v_ilev_rc_door2',
				objYaw = -143.0,
				objCoords = vector3(-1057.18, -839.44, 5.14)
			}
		}
	},

	{
		objName = 'v_ilev_rc_door2',
		objYaw = 38.0,
		objCoords  = vector3(-1108.40, -842.35, 14.68),
		textCoords = vector3(-1108.12, -842.08, 13.68),
		authorizedJobs = { 'police', 'emspolice' },
		locking = false,
		locked = true,
		pickable = false,
		distance = 1.5,
	},

	{
		textCoords = vector3(-1094.78, -844.50, 20.0),
		authorizedJobs = { 'police', 'emspolice' },
		locking = false,
		locked = true,
		pickable = false,
		distance = 2.5,
		doors = {
			{
				objName = 'vesp_door1',
				objYaw = -233.3,
				objCoords = vector3(-1094.99, -844.11, 19.0)
			},

			{
				objName = 'vesp_door1',
				objYaw = -52.0,
				objCoords = vector3(-1094.47, -844.87, 19.0)
			}
		}
	},

	{
		textCoords = vector3(434.81, -981.93, 31.65),
		authorizedJobs = { 'police', 'emspolice' },
		locking = false,
		locked = false,
		pickable = false,
		distance = 2.5,
		doors = {
			{
				objName = 'v_ilev_ph_door01',
				objYaw = -90.0,
				objCoords = vector3(434.7, -980.6, 30.8)
			},

			{
				objName = 'v_ilev_ph_door002',
				objYaw = -90.0,
				objCoords = vector3(434.7, -983.2, 30.8)
			}
		}
	},
	{
		objName = 'v_ilev_ph_gendoor004',
		objYaw = 90.0,
		objCoords  = vector3(450.05, -986.36, 31.6),
		textCoords = vector3(450.09, -986.92, 30.68),
		authorizedJobs = { 'police', 'emspolice' },
		locking = false,
		locked = true,
		pickable = false,
		distance = 1.5,
	},
	-- Rooftop
	{
		objName = 'v_ilev_gtdoor02',
		objYaw = 90.0,
		objCoords  = vector3(464.23, -984.01, 44.8),
		textCoords = vector3(464.38, -983.64, 43.8),
		authorizedJobs = { 'police', 'emspolice' },
		locking = false,
		locked = true,
		pickable = false,
		distance = 1.5,
	},
	-- Hallway to roof
	{
		objName = 'v_ilev_arm_secdoor',
		objYaw = 90.0,
		objCoords  = vector3(461.23, -985.97, 31.8),
		textCoords = vector3(461.34, -986.283, 30.8),
		authorizedJobs = { 'police', 'emspolice' },
		locking = false,
		locked = true,
		pickable = false,
		distance = 1.5,
	},
	-- Armory
	{
		objName = 'v_ilev_arm_secdoor',
		objYaw = -90.0,
		objCoords  = vector3(453.06, -982.59, 31.6), 
		textCoords = vector3(452.95, -982.16, 30.99),
		authorizedJobs = { 'police', 'emspolice' },
		locking = false,
		locked = true,
		pickable = false,
		distance = 1.5,
	},
	-- Captain Office
	{
		objName = 'v_ilev_ph_gendoor002',
		objYaw = -180.0,
		objCoords  = vector3(447.25, -980.1, 31.6),
		textCoords = vector3(447.61, -979.9, 30.7),
		authorizedJobs = { 'police', 'emspolice' },
		locking = false,
		locked = true,
		pickable = false,
		distance = 1.5,
	},
	-- To downstairs (double doors)
	{
		textCoords = vector3(444.71, -989.43, 31.60),
		authorizedJobs = { 'police', 'emspolice' },
		locking = false,
		locked = true,
		pickable = false,
		distance = 2.0,
		doors = {
			{
				objName = 'v_ilev_ph_gendoor005',
				objYaw = 180.0,
				objCoords = vector3(443.9, -989.0, 30.6)
			},

			{
				objName = 'v_ilev_ph_gendoor005',
				objYaw = 0.0,
				objCoords = vector3(445.3, -988.7, 30.6)
			}
		}
	},
	--
	-- Mission Row Cells
	--
	-- Main Cells
	{
		objName = 'v_ilev_ph_cellgate',
		objYaw = 0.0,
		objCoords  = vector3(464.0, -992.67, 25.9),
		textCoords = vector3(463.3, -992.6, 25.1),
		authorizedJobs = { 'police', 'emspolice' },
		locking = false,
		locked = true,
		pickable = false,
		distance = 1.3,
	},
	-- Cell 1
	{
		objName = 'v_ilev_ph_cellgate',
		objYaw = -90.0,
		objCoords  = vector3(461.8, -993.73, 25.9),
		textCoords = vector3(461.8, -993.3, 25.0),
		authorizedJobs = { 'police', 'emspolice' },
		locking = false,
		locked = true,
		pickable = false,
		distance = 1.3,
	},
	-- Cell 2
	{
		objName = 'v_ilev_ph_cellgate',
		objYaw = 90.0,
		objCoords  = vector3(462.3, -998.1, 24.9),
		textCoords = vector3(461.8, -998.8, 25.0),
		authorizedJobs = { 'police', 'emspolice' },
		locking = false,
		locked = true,
		pickable = false,
		distance = 1.0,
	},
	-- Cell 3
	{
		objName = 'v_ilev_ph_cellgate',
		objYaw = 90.0,
		objCoords  = vector3(462.7, -1001.9, 24.9),
		textCoords = vector3(461.8, -1002.4, 25.0),
		authorizedJobs = { 'police', 'emspolice' },
		locking = false,
		locked = true,
		pickable = false,
		distance = 1.0,
	},
	-- To Back
	{
		objName = 'v_ilev_gtdoor',
		objYaw = 0.0,
		objCoords  = vector3(464.61, -1003.64, 24.98),
		textCoords = vector3(464.61, -1003.64, 24.98),
		authorizedJobs = { 'police', 'emspolice' },
		locking = false,
		locked = true,
		pickable = false,
		distance = 1.0,
	},
	--
	-- Mission Row Back
	--
	-- Back (double doors)
	{
		textCoords = vector3(468.67, -1014.43, 26.48),
		authorizedJobs = { 'police', 'emspolice' },
		locking = false,
		locked = true,
		pickable = false,
		distance = 2.5,
		doors = {
			{
				objName = 'v_ilev_rc_door2',
				objYaw = 0.0,
				objCoords  = vector3(467.3, -1014.4, 26.5)
			},

			{
				objName = 'v_ilev_rc_door2',
				objYaw = 180.0,
				objCoords  = vector3(469.9, -1014.4, 26.5)
			}
		}
	},
	-- Back Gate
	{
		objName = 'hei_prop_station_gate',
		objYaw = 90.0,
		objCoords  = vector3(488.8, -1017.2, 27.1),
		textCoords = vector3(488.8, -1020.2, 30.0),
		authorizedJobs = { 'police', 'emspolice' },
		locking = false,
		locked = true,
		pickable = false,
		distance = 14,
		size = 2
	},
	--
	-- Mission Row Extension
	--
	-- Briefing room
	{
		textCoords = vector3(455.86, -981.31, 26.86),
		authorizedJobs = { 'police', 'emspolice' },
		locking = false,
		locked = false,
		pickable = false,
		distance = 2.5,
		doors = {
			{
				objName = 'v_ilev_ph_gendoor005',
				objYaw = 90.0,
				objCoords  = vector3(455.94, -980.57, 26.67)
			},

			{
				objName = 'v_ilev_ph_gendoor005',
				objYaw = 270.0,
				objCoords  = vector3(455.85, -982.14, 26.67)
			}
		}
	},
	-- To Breakrooms
	{
		objName = 'v_ilev_ph_gendoor005',
		objYaw = 90.0,
		objCoords  = vector3(465.62, -985.93, 25.74),
		textCoords = vector3(465.62, -985.93, 25.74),
		authorizedJobs = { 'police', 'emspolice' },
		locking = false,
		locked = true,
		pickable = false,
		distance = 1.0,
	},
	-- New Cell 1
	{
		objName = 'v_ilev_ph_cellgate',
		objYaw = 180.0,
		objCoords  = vector3(468.57, -999.44, 25.07),
		textCoords = vector3(468.57, -999.44, 25.07),
		authorizedJobs = { 'police', 'emspolice' },
		locking = false,
		locked = true,
		pickable = false,
		distance = 1.0,
	},
	-- New Cell 2
	{
		objName = 'v_ilev_ph_cellgate',
		objYaw = 180.0,
		objCoords  = vector3(472.16, -999.47, 25.05),
		textCoords = vector3(472.16, -999.47, 25.05),
		authorizedJobs = { 'police', 'emspolice' },
		locking = false,
		locked = true,
		pickable = false,
		distance = 1.0,
	},
	-- New Cell 3
	{
		objName = 'v_ilev_ph_cellgate',
		objYaw = 180.0,
		objCoords  = vector3(476.4, -1007.68, 24.41),
		textCoords = vector3(476.4, -1007.68, 24.41),
		authorizedJobs = { 'police', 'emspolice' },
		locking = false,
		locked = true,
		pickable = false,
		distance = 1.0,
	},
	-- New Cell 4
	{
		objName = 'v_ilev_ph_cellgate',
		objYaw = 180.0,
		objCoords  = vector3(480.0, -1007.7, 24.41),
		textCoords = vector3(480.0, -1007.7, 24.41),
		authorizedJobs = { 'police', 'emspolice' },
		locking = false,
		locked = true,
		pickable = false,
		distance = 1.0,
	},
	-- To Photoroom
	{
		objName = 'v_ilev_ph_gendoor006',
		objYaw = 0.0,
		objCoords  = vector3(475.05, -995.08, 24.46),
		textCoords = vector3(475.05, -995.08, 24.46),
		authorizedJobs = { 'police', 'emspolice' },
		locking = false,
		locked = true,
		pickable = false,
		distance = 1.0,
	},
	-- To Standplace
	{
		objName = 'v_ilev_ph_gendoor006',
		objYaw = 180.0,
		objCoords  = vector3(485.71, -996.08, 24.45),
		textCoords = vector3(485.71, -996.08, 24.45),
		authorizedJobs = { 'police', 'emspolice' },
		locking = false,
		locked = true,
		pickable = false,
		distance = 1.0,
	},
	-- To Interigation 1
	{
		objName = 'v_ilev_ph_gendoor005',
		objYaw = 180.0,
		objCoords  = vector3(484.63, -999.74, 24.46),
		textCoords = vector3(484.63, -999.74, 24.46),
		authorizedJobs = { 'police', 'emspolice' },
		locking = false,
		locked = true,
		pickable = false,
		distance = 1.0,
	},
	-- To Interigation 2
	{
		objName = 'v_ilev_ph_gendoor005',
		objYaw = 180.0,
		objCoords  = vector3(491.37, -999.81, 24.46),
		textCoords = vector3(491.37, -999.81, 24.46),
		authorizedJobs = { 'police', 'emspolice' },
		locking = false,
		locked = true,
		pickable = false,
		distance = 1.0,
	},
	-- To Evidence
	{
		objName = 'v_ilev_ph_gendoor005',
		objYaw = 180.0,
		objCoords  = vector3(470.78, -992.21, 25.12),
		textCoords = vector3(470.78, -992.21, 25.12),
		authorizedJobs = { 'police', 'emspolice' },
		locking = false,
		locked = true,
		pickable = false,
		distance = 1.0,
	},
	
	-- Bureau Paleto Bay
	{
		textCoords = vector3(-435.57, 6008.76, 27.98),
		authorizedJobs = { 'police', 'emspolice' },
		locking = false,
		locked = true,
		pickable = false,
		distance = 2.5,
		doors = {
			{
				objName = 'v_ilev_ph_gendoor002',
				objYaw = -135.0,
				objCoords = vector3(-436.5157, 6007.844, 28.13839),			
			},
			{
				objName = 'v_ilev_ph_gendoor002',
				objYaw = 45.0,
				objCoords = vector3(-434.6776, 6009.681, 28.13839)			
			}
		}
	},	
	-- Achterdeur links
	{
		objName = 'v_ilev_rc_door2',
		objYaw = 135.0,
		objCoords  = vector3(-450.9664, 6006.086, 31.99004),		
		textCoords = vector3(-451.38, 6006.55, 31.84),
		authorizedJobs = { 'police', 'emspolice' },
		locking = false,
		locked = true,
		pickable = false,
		distance = 2.0,
	},
	-- Achterdeur rechts
	{
		objName = 'v_ilev_rc_door2',
		objYaw = -45.0,
		objCoords  = vector3(-447.2363, 6002.317, 31.84003),		
		textCoords = vector3(-446.77, 6001.84, 31.68),
		authorizedJobs = { 'police', 'emspolice' },
		locking = false,
		locked = true,
		pickable = false,
		distance = 2.0,
	},
	-- Kleedkamer
	{
		objName = 'v_ilev_rc_door2',
		objYaw = -45.0,
		objCoords  = vector3(-450.7136, 6016.371, 31.86523),				
		textCoords = vector3(-450.15, 6015.96, 31.71),
		authorizedJobs = { 'police', 'emspolice' },
		locking = false,
		locked = true,
		pickable = false,
		distance = 2.0,
	},	
	-- Kleedkamer 2
	{
		objName = 'v_ilev_rc_door2',
		objYaw = 45.0,
		objCoords  = vector3(-454.0435, 6010.243, 31.86106),						
		textCoords = vector3(-453.56, 6010.73, 31.71),
		authorizedJobs = { 'police', 'emspolice' },
		locking = false,
		locked = true,
		pickable = false,
		distance = 2.0,
	},	
	-- Wapenkamer
	{
		objName = 'v_ilev_arm_secdoor',
		objYaw = -135.0,
		objCoords  = vector3(-439.1576, 5998.157, 31.86815),						
		textCoords = vector3(-438.64, 5998.51, 31.71), 
		authorizedJobs = { 'police', 'emspolice' },
		locking = false,
		locked = true,
		pickable = false,
		distance = 2.0,
	},	
	-- Verhoorkamer
	{
		objName = 'v_ilev_arm_secdoor',
		objYaw = 45.0,
		objCoords  = vector3(-436.6276, 6002.548, 28.14062),							
		textCoords = vector3(-437.09, 6002.100, 27.98),
		authorizedJobs = { 'police', 'emspolice' },
		locking = false,
		locked = true,
		pickable = false,
		distance = 2.0,
	},
	-- Entree cellen 1
	{
		objName = 'v_ilev_ph_cellgate1',
		objYaw = 45.0,
		objCoords  = vector3(-438.228, 6006.167, 28.13558),							
		textCoords = vector3(-438.610, 6005.64, 27.98),
		authorizedJobs = { 'police', 'emspolice' },
		locking = false,
		locked = true,
		pickable = false,
		distance = 2.0,
	},
	-- Entree cellen 2
	{
		objName = 'v_ilev_ph_cellgate1',
		objYaw = 45.0,
		objCoords  = vector3(-442.1082, 6010.052, 28.13558),							
		textCoords = vector3(-442.55, 6009.61, 27.98),
		authorizedJobs = { 'police', 'emspolice' },
		locking = false,
		locked = true,
		pickable = false,
		distance = 2.0,
	},	
	-- Cel
	{
		objName = 'v_ilev_ph_cellgate1',
		objYaw = 45.0,
		objCoords  = vector3(-444.3682, 6012.223, 28.13558),								
		textCoords = vector3(-444.77, 6011.74, 27.98),
		authorizedJobs = { 'police', 'emspolice' },
		locking = false,
		locked = true,
		pickable = false,
		distance = 2.0,
	},						
	-- Gang lockers (dubbele deuren)
	{
		textCoords = vector3(-442.09, 6011.93, 31.86523),
		authorizedJobs = { 'police', 'emspolice' },
		locking = false,
		locked = true,
		pickable = false,
		distance = 2.5,
		doors = {
			{
				objName = 'v_ilev_rc_door2',
				objYaw = 225.0,
				objCoords  = vector3(-441.0185, 6012.795, 31.86523),			
			},

			{
				objName = 'v_ilev_rc_door2',
				objYaw = 45.0,
				objCoords  = vector3(-442.8578, 6010.958, 31.86523)			
			}
		}
	},	
	-- Gang naar achterkant (dubbele deuren)
	{
		textCoords = vector3(-448.67, 6007.52, 31.86523),
		authorizedJobs = { 'police', 'emspolice' },
		locking = false,
		locked = true,
		pickable = false,
		distance = 2.5,
		doors = {
			{
				objName = 'v_ilev_rc_door2',
				objYaw = 135.0,
				objCoords = vector3(-447.7283, 6006.702, 31.86523),				
			},

			{
				objName = 'v_ilev_rc_door2',
				objYaw = -45.0,
				objCoords = vector3(-449.5656, 6008.538, 31.86523)		
			}
		}
	},				
	--
	-- Bolingbroke Penitentiary
	--
	-- Entrance (Two big gates)
	{
		objName = 'prop_gate_prison_01',
		objCoords  = vector3(1844.9, 2604.8, 44.6),
		textCoords = vector3(1844.9, 2608.5, 48.0),
		authorizedJobs = { 'police', 'emspolice' },
		locking = false,
		locked = true,
		pickable = false,
		distance = 13,
		size = 2
	},
	{
		objName = 'prop_gate_prison_01',
		objCoords  = vector3(1818.5, 2604.8, 44.6),
		textCoords = vector3(1818.5, 2608.4, 48.0),
		authorizedJobs = { 'police', 'emspolice' },
		locking = false,
		locked = true,
		pickable = false,
		distance = 13,
		size = 2
	},
	{
		objName = 'prop_gate_prison_01',
		objCoords = vector3(1799.237, 2616.303, 44.6),
		textCoords = vector3(1795.941, 2616.969, 48.0),
		authorizedJobs = { 'police', 'emspolice' },
		locking = false,
		locked = true,
		pickable = false,
		distance = 10,
		size = 2
	},


	--outside doors
	{
		objName = 'prop_fnclink_03gate5',
		objCoords = vector3(1796.322, 2596.574, 45.565),
		textCoords = vector3(1796.322, 2596.574, 45.965),
		authorizedJobs = { 'police', 'emspolice' },
		locking = false,
		locked = true,
		pickable = false,
		distance = 2.5,
		size = 2
	},
	{
		objName = 'v_ilev_gtdoor',
		objCoords = vector3(1821.677, 2477.265, 45.945),
		textCoords = vector3(1821.677, 2477.265, 45.945),
		authorizedJobs = { 'police', 'emspolice' },
		locking = false,
		locked = true,
		pickable = false,
		distance = 2.5,
		size = 2
	},
	{
		objName = 'v_ilev_gtdoor',
		objCoords = vector3(1760.692, 2413.251, 45.945),
		textCoords = vector3(1760.692, 2413.251, 45.945),
		authorizedJobs = { 'police', 'emspolice' },
		locking = false,
		locked = true,
		pickable = false,
		distance = 2.5,
		size = 2
	},
	{
		objName = 'v_ilev_gtdoor',
		objCoords = vector3(1543.58, 2470.252, 45.945),
		textCoords = vector3(1543.58, 2470.25, 45.945),
		authorizedJobs = { 'police', 'emspolice' },
		locking = false,
		locked = true,
		pickable = false,
		distance = 2.5,
		size = 2
	},
	{
		objName = 'v_ilev_gtdoor',
		objCoords = vector3(1659.733, 2397.475, 45.945),
		textCoords = vector3(1659.733, 2397.475, 45.945),
		authorizedJobs = { 'police', 'emspolice' },
		locking = false,
		locked = true,
		pickable = false,
		distance = 2.5,
		size = 2
	},
	{
		objName = 'v_ilev_gtdoor',
		objCoords = vector3(1537.731, 2584.842, 45.945),
		textCoords = vector3(1537.731, 2584.842, 45.945),
		authorizedJobs = { 'police', 'emspolice' },
		locking = false,
		locked = true,
		pickable = false,
		distance = 2.5,
		size = 2
	},
	{
		objName = 'v_ilev_gtdoor',
		objCoords  = vector3(1571.964, 2678.354, 45.945),
		textCoords = vector3(1571.964, 2678.354, 45.945),
		authorizedJobs = { 'police', 'emspolice' },
		locking = false,
		locked = true,
		pickable = false,
		distance = 2.5,
		size = 2
	},
	{
		objName = 'v_ilev_gtdoor',
		objCoords  = vector3(1650.18, 2755.104, 45.945),
		textCoords = vector3(1650.18, 2755.104, 45.945),
		authorizedJobs = { 'police', 'emspolice' },
		locking = false,
		locked = true,
		pickable = false,
		distance = 2.5,
		size = 2
	},
	{
		objName = 'v_ilev_gtdoor',
		objCoords  = vector3(1771.98, 2759.98, 45.945),
		textCoords = vector3(1771.98, 2759.98, 45.945),
		authorizedJobs = { 'police', 'emspolice' },
		locking = false,
		locked = true,
		pickable = false,
		distance = 2.5,
		size = 2
	},
	{
		objName = 'v_ilev_gtdoor',
		objCoords  = vector3(1845.7, 2699.79, 45.945),
		textCoords = vector3(1845.7, 2699.79, 45.945),
		authorizedJobs = { 'police', 'emspolice' },
		locking = false,
		locked = true,
		pickable = false,
		distance = 2.5,
		size = 2
	},
	{
		objName = 'v_ilev_gtdoor',
		objCoords  = vector3(1820.68, 2621.95, 45.945),
		textCoords = vector3(1820.68, 2621.95, 45.945),
		authorizedJobs = { 'police', 'emspolice' },
		locking = false,
		locked = true,
		pickable = false,
		distance = 2.5,
		size = 2
	},
	--
	-- Bolingbroke Extra
	--
	-- To Offices
	{
		objName = 'v_ilev_gtdoor',
		objYaw = 90.0,
		objCoords  = vector3(1819.129, 2593.64, 46.09929),
		textCoords = vector3(1843.3, 2579.39, 45.98),
		authorizedJobs = { 'police', 'emspolice' },
		locking = false,
		locked = true,
		pickable = false,
		distance = 2.0,
		size = 2
	},

	-- To Changingroom
	{
		objName = 'v_ilev_gtdoor',
		objYaw = 360.0,
		objCoords  = vector3(1827.365, 2587.547, 46.09929),
		textCoords = vector3(1835.76, 2583.15, 45.95),
		authorizedJobs = { 'police', 'emspolice' },
		locking = false,
		locked = true,
		pickable = false,
		distance = 2.0,
		size = 2
	},
	-- To CrimChangingroom
	{
		objName = 'v_ilev_gtdoor',
		objYaw = 90.0,
		objCoords  = vector3(1826.466, 2585.271, 46.09929),
		textCoords = vector3(1835.77, 2589.76, 45.95),
		authorizedJobs = { 'police', 'emspolice' },
		locking = false,
		locked = true,
		pickable = false,
		distance = 1.0,
		size = 2
	},
	-- To CheckingRoom
	{
		objName = 'v_ilev_gtdoor',
		objYaw = 0.0,
		objCoords  = vector3(1827.521, 2583.905, 45.28576),
		textCoords = vector3(1828.638, 2584.675, 45.95233),
		authorizedJobs = { 'police', 'emspolice' },
		locking = false,
		locked = true,
		pickable = false,
		distance = 2,
		size = 2
	},
	-- Checking Gate
	{
		objName = 'v_ilev_gtdoor',
		objYaw = 270.0,
		objCoords  = vector3(1837.714, 2595.185, 46.09929),
		textCoords = vector3(1837.714, 2595.185, 46.09929),
		authorizedJobs = { 'police', 'emspolice' },
		locking = false,
		locked = true,
		pickable = false,
		distance = 1.5,
		size = 2
	},
	-- To CheckingRoomFromCheck
	{
		objName = 'v_ilev_gtdoor',
		objYaw = 90.0,
		objCoords  = vector3(1837.697, 2585.24, 46.09929),
		textCoords = vector3(1837.697, 2585.24, 46.09929),
		authorizedJobs = { 'police', 'emspolice' },
		locking = false,
		locked = true,
		pickable = false,
		distance = 1.5,
		size = 2
	},
	-- To SecondCheckGate
	
	-- 58
	-- X:1845.198 Y:2585.24 Z:46.09929
	{
		objName = 'v_ilev_gtdoor',
		objYaw = 90.0,
		objCoords  = vector3(1845.198, 2585.24, 46.09929),
		textCoords = vector3(1845.198, 2585.24, 46.09929),
		authorizedJobs = { 'police', 'emspolice' },
		locking = false,
		locked = false,
		pickable = false,
		distance = 1.5,
		size = 2
	},
	-- To MainHall
	{
		objName = 'v_ilev_ph_door002',
		objYaw = 90.0,
		objCoords  = vector3(1791.18, 2593.11, 546.15),
		textCoords = vector3(1791.18, 2593.11, 546.15),
		authorizedJobs = { 'police', 'emspolice' },
		locking = false,
		locked = true,
		pickable = false,
		distance = 2.0,
		size = 2
	},
	-- To VisitorRoom
	{
		objName = 'prison_prop_door2',
		objYaw = 90.0,
		objCoords  = vector3(1784, 2599, 46),
		textCoords = vector3(1785.808, 2590.02, 44.79703),
		authorizedJobs = { 'police', 'emspolice' },
		locking = false,
		locked = true,
		pickable = false,
		distance = 2.0,
		size = 2
	},
	{
		objName = 'prison_prop_door1',
		objYaw = 0.0,
		objCoords  = vector3(1786, 2600, 46),
		textCoords = vector3(1785.808, 2590.02, 44.79703),
		authorizedJobs = { 'police', 'emspolice' },
		locking = false,
		locked = true,
		pickable = false,
		distance = 2.0,
		size = 2
	},
	{
		textCoords = vector3(1785.83, 2609.59, 45.99),
		authorizedJobs = { 'police', 'emspolice' },
		locking = false,
		locked = true,
		pickable = false,
		distance = 2.5,
		doors = {
			{
				objName = 'prison_prop_door1',
				objYaw = 182.0,
				objCoords = vector3(1785, 2610, 46),	
			},

			{
				objName = 'prison_prop_door1a',
				objYaw = 0.0,
				objCoords = vector3(1787, 2610, 46),
			}
		}
	},
	{
		textCoords = vector3(1779.67, 2601.83, 50.71),
		authorizedJobs = { 'police', 'emspolice' },
		locking = false,
		locked = true,
		pickable = false,
		distance = 2.5,
		doors = {
			{
				objName = 'prison_prop_door2',
				objYaw = 1.5,
				objCoords = vector3(1781, 2602, 51),	
			},

			{
				objName = 'prison_prop_door2',
				objYaw = 180.0,
				objCoords = vector3(1778, 2602, 51),
			}
		}
	},
	{
		objName = 'prison_prop_door2',
		objYaw = 0.0,
		objCoords  = vector3(1780, 2596, 51),
		textCoords = vector3(1785.808, 2590.02, 44.79703),
		authorizedJobs = { 'police', 'emspolice' },
		locking = false,
		locked = true,
		pickable = false,
		distance = 2.0,
		size = 2
	},
	{
		objName = 'prison_prop_door1',
		objYaw = 0.0,
		objCoords  = vector3(1787, 2621, 46),
		textCoords = vector3(1785.808, 2590.02, 44.79703),
		authorizedJobs = { 'police', 'emspolice' },
		locking = false,
		locked = true,
		pickable = false,
		distance = 2.0,
		size = 2
	},
	{
		objName = 'prison_prop_door2',
		objYaw = 270.0,
		objCoords  = vector3(1788, 2606, 51),
		textCoords = vector3(1785.808, 2590.02, 44.79703),
		authorizedJobs = { 'police', 'emspolice' },
		locking = false,
		locked = true,
		pickable = false,
		distance = 2.0,
		size = 2
	},
	{
		textCoords = vector3(1791.08, 2593.76, 46.18),
		authorizedJobs = { 'police', 'emspolice' },
		locking = false,
		locked = true,
		pickable = false,
		distance = 2.5,
		doors = {
			{
				objName = 'prison_prop_door1',
				objYaw = 272.0,
				objCoords = vector3(1791.1140136719, 2592.50390625, 46.3132473297119),		
			},

			{
				objName = 'prison_prop_door1a',
				objYaw = 90.0,
				objCoords = vector3(1791, 2595, 46),	
			}
		}
	},	
	-- To DoctorRoom
	{
		objName = 'v_ilev_ph_gendoor002',
		objYaw = 90.0,
		objCoords  = vector3(1786.4, 2579.8, 45.97),
		textCoords = vector3(1786.4, 2579.8, 45.97),
		authorizedJobs = { 'police', 'emspolice' },
		locking = false,
		locked = true,
		pickable = false,
		distance = 1.0,
		size = 2
	},
	-- HallGate
	{
		objName = 'prison_prop_door2',
		objYaw = 0.0,
		objCoords  = vector3(1786, 2567, 46),
		textCoords = vector3(1778.91, 2568.91, 46.07),
		authorizedJobs = { 'police', 'emspolice' },
		locking = false,
		locked = true,
		pickable = false,
		distance = 2.0,
		size = 2
	},
	-- HallGate 2
	{
		objName = 'prison_prop_door1',
		objYaw = 270.0,
		objCoords  = vector3(1792, 2551, 46),
		textCoords = vector3(1773.49, 2568.9, 46.07),
		authorizedJobs = { 'police', 'emspolice' },
		locking = false,
		locked = true,
		pickable = false,
		distance = 2.0,
		size = 2
	},
	{
		textCoords = vector3( 1781.72, 2552.07, 49.57),
		authorizedJobs = { 'police', 'emspolice' },
		locking = false,
		locked = true,
		pickable = false,
		distance = 2.5,
		doors = {
			{
				objName = 'prison_prop_door2',
				objYaw = 269.5,
				objCoords = vector3(1782, 2551, 50),		
			},

			{
				objName = 'prison_prop_door2',
				objYaw = 90.0,
				objCoords = vector3(1782, 2553, 50),	
			}
		}
	},	
	-- Gate To Work
	{
		objName = 'prison_prop_door2',
		objYaw = 90.0,
		objCoords  = vector3(1786, 2552, 50),
		textCoords = vector3(1760.89, 2578.51, 46.07),
		authorizedJobs = { 'police', 'emspolice' },
		locking = false,
		locked = true,
		pickable = false,
		distance = 2.0,
		size = 2
	},
	-- Cell Maindoor
	{
		objName = 'prison_prop_door2',
		objYaw = 180.0,
		objCoords  = vector3(1785, 2550, 46),
		textCoords = vector3(1760.89, 2578.51, 46.07),
		authorizedJobs = { 'police', 'emspolice' },
		locking = false,
		locked = true,
		pickable = false,
		distance = 2.0,
		size = 2
	},
	{
		objName = 'prison_prop_door1a',
		objYaw = 270.0,
		objCoords  = vector3(1776, 2551, 46),
		textCoords = vector3(1760.89, 2578.51, 46.07),
		authorizedJobs = { 'police', 'emspolice' },
		locking = false,
		locked = true,
		pickable = false,
		distance = 2.0,
		size = 2
	},
	----------------
	-- Pacific Bank
	----------------
	-- First Door
	--[[{
		text = '(Use Lockpick)',
		objName = 'hei_v_ilev_bk_gate_pris',			
		objCoords  = vector3(257.41, 220.25, 106.4),
		textCoords = vector3(257.41, 220.25, 106.4),
		authorizedJobs = { 'police', 'emspolice' },
		objYaw = -20.0,
		locking = false,
		locked = true,
		pickable = true,
		distance = 1.5,
		size = 2
	},
	-- Second Door
	{
		objName = 'hei_v_ilev_bk_gate2_pris',
		objCoords  = vector3(261.83, 221.39, 106.41),
		textCoords = vector3(261.83, 221.39, 106.41),
		authorizedJobs = { 'police', 'emspolice' },
		objYaw = -110.0,
		locking = false,
		locked = true,
		pickable = false,
		distance = 1.5,
		size = 2
	},
	-- Office to gate door
	{
		text = '(Use Lockpick)',
		objName = 'v_ilev_bk_door',
		objCoords  = vector3(265.19, 217.84, 110.28),
		textCoords = vector3(265.19, 217.84, 110.28),
		authorizedJobs = { 'police', 'emspolice' },
		objYaw = -20.0,
		locking = false,
		locked = true,
		pickable = true,
		distance = 1.5,
		size = 2
	},

	-- First safe Door
	{
		objName = 'hei_v_ilev_bk_safegate_pris',
		objCoords  = vector3(252.98, 220.65, 101.8),
		textCoords = vector3(252.98, 220.65, 101.8),
		authorizedJobs = { 'police', 'emspolice' },
		objYaw = 160.0,
		locking = false,
		locked = true,
		pickable = false,
		distance = 1.5,
		size = 2
	},
	-- Second safe Door
	{
		objName = 'hei_v_ilev_bk_safegate_pris',
		objCoords  = vector3(261.68, 215.62, 101.81),
		textCoords = vector3(261.68, 215.62, 101.81),
		authorizedJobs = { 'police', 'emspolice' },
		objYaw = -110.0,
		locking = false,
		locked = true,
		pickable = false,
		distance = 1.5,
		size = 2
	},

	----------------
	-- Fleeca Banks
	----------------
	-- Door 1
	{
		text = '(Use Lockpick)',
		objName = 'v_ilev_gb_vaubar',
		objCoords  = vector3(314.61, -285.82, 54.49),
		textCoords = vector3(313.3, -285.45, 54.49),
		authorizedJobs = { 'police', 'emspolice' },
		locking = false,
		locked = true,
		pickable = true,
		distance = 1.5,
		size = 2
	},
	-- Door 2
	{
		text = '(Use Lockpick)',
		objName = 'v_ilev_gb_vaubar',
		objCoords  = vector3(148.96, -1047.12, 29.7),
		textCoords = vector3(148.96, -1047.12, 29.7),
		authorizedJobs = { 'police', 'emspolice' },
		locking = false,
		locked = true,
		pickable = true,
		distance = 1.5,
		size = 2
	},
	-- Door 3
	{
		text = '(Use Lockpick)',
		objName = 'v_ilev_gb_vaubar',
		objCoords  = vector3(-351.7, -56.28, 49.38),
		textCoords = vector3(-351.7, -56.28, 49.38),
		authorizedJobs = { 'police', 'emspolice' },
		locking = false,
		locked = true,
		pickable = true,
		distance = 1.5,
		size = 2
	},
	-- Door 4
	{
		text = '(Use Lockpick)',
		objName = 'v_ilev_gb_vaubar',
		objCoords  = vector3(-2956.18, -335.76, 38.11),
		textCoords = vector3(-2956.18, -335.76, 38.11),
		authorizedJobs = { 'police', 'emspolice' },
		locking = false,
		locked = true,
		pickable = true,
		distance = 1.5,
		size = 2
	},
	-- Door 5
	{
		text = '(Use Lockpick)',
		objName = 'v_ilev_gb_vaubar',
		objCoords  = vector3(-2956.18, 483.96, 16.02),
		textCoords = vector3(-2956.18, 483.96, 16.02),
		authorizedJobs = { 'police', 'emspolice' },
		locking = false,
		locked = true,
		pickable = true,
		distance = 1.5,
		size = 2
	},
	-- Paleto Door 1
	{
		objName = 'v_ilev_cbankvaulgate01',
		objCoords  = vector3(-105.77, 6472.59, 31.81),
		textCoords = vector3(-105.77, 6472.59, 31.81),
		objYaw = 45.0,
		authorizedJobs = { 'police', 'emspolice' },
		locking = false,
		locked = true,
		pickable = false,
		distance = 1.5,
		size = 2
	},
	-- Paleto Door 2
	{
		objName = 'v_ilev_cbankvaulgate02',
		objCoords  = vector3(-106.26, 6476.01, 31.98),
		textCoords = vector3(-105.5, 6475.08, 31.99),
		objYaw = -45.0,
		authorizedJobs = { 'police', 'emspolice' },
		locking = false,
		locked = true,
		pickable = false,
		distance = 1.5,
		size = 2
	},
	-----
	-- Police front gate
	-----
	--[[{
		objName = 'prop_facgate_07b',
		objYaw = -90.0,
		objCoords  = vector3(419.99, -1025.0, 28.99),
		textCoords = vector3(419.9, -1021.04, 30.5),
		authorizedJobs = { 'police', 'emspolice' },
		locking = false,
		locked = true,
		pickable = false,
		distance = 20,
		size = 2-----------
	},]]--------
	-----
	-- Luxury Cars
	-----
	-- Entrance Doors 
	--[[{
		textCoords = vector3(-803.0223, -223.8222, 37.87975),
		authorizedJobs = { 'cardealer', 'police', 'emspolice' },
		locking = false,
		locked = true,
		pickable = false,
		distance = 3.5,
		doors = {
			{
				objName = 'prop_doorluxyry2',
				objYaw = 120.0,
				objCoords = vector3(-803.0223, -222.5841, 37.87975)
			},

			{
				objName = 'prop_doorluxyry2',
				objYaw = -60.0,
				objCoords = vector3(-801.9622, -224.5203, 37.87975)			
			}
		}
	},	
	-- Side Entrance Doors 
	{
		textCoords = vector3(-777.1855, -244.0013, 37.333889),
		authorizedJobs = { 'cardealer', 'police', 'emspolice' },
		locking = false,
		locked = true,
		pickable = false,
		distance = 3.5,
		doors = {
			{
				objName = 'prop_doorluxyry',				
				objYaw = -160.0,
				objCoords = vector3(-778.1855, -244.3013, 37.33388)
			},

			{
				objName = 'prop_doorluxyry',
				objYaw = 23.0,
				objCoords = vector3(-776.1591, -243.5013, 37.33388)				
			}
		}
	},	
	-- Garage Doors
	{
		textCoords = vector3(-768.1264, -238.9737, 37.43247),
		authorizedJobs = { 'cardealer', 'police', 'emspolice' },
		locking = false,
		locked = true,
		pickable = false,
		distance = 13.0,
		doors = {
			{
				objName = 'prop_autodoor',
				objCoords = vector3(-770.6311, -240.0069, 37.43247)    
			},

			{
				objName = 'prop_autodoor',
				objCoords = vector3(-765.6217, -237.9405, 37.43247) 		
			}
		}
	},		]]--
	----------------
	-- Pickle Rental
	----------------
	-- Voordeur 1
	--[[{
		objName = 'apa_prop_apa_cutscene_doorb',
		objCoords  = vector3(-21.71276, -1392.778, 29.63847),		
		textCoords = vector3(-22.31276, -1392.778, 29.63847),
		authorizedJobs = { 'cardealer' },
		objYaw = -180.0,
		locking = false,
		locked = true,
		pickable = false,
		distance = 2.5,
		size = 2
	},	
	-- Voordeur 2
	{
		objName = 'apa_prop_apa_cutscene_doorb',
		objCoords  = vector3(-32.67987, -1392.064, 29.63847),		
		textCoords = vector3(-32.10987, -1392.064, 29.63847),
		authorizedJobs = { 'cardealer' },
		objYaw = 0.0,
		locking = false,
		locked = true,
		pickable = false,
		distance = 2.5,
		size = 2
	},	
	-- Deur naar kelder
	{
		objName = 'apa_prop_apa_cutscene_doorb',
		objCoords  = vector3(-24.22668, -1403.067, 29.63847),				
		textCoords = vector3(-24.22668, -1402.537, 29.63847),
		authorizedJobs = { 'cardealer' },
		objYaw = 90.0,
		locking = false,
		locked = true,
		pickable = false,
		distance = 1.5,
		size = 2
	},	
	-- Achterdeur
	{
		objName = 'apa_prop_apa_cutscene_doorb',
		objCoords  = vector3(-21.27107, -1406.845, 29.63847),			
		textCoords = vector3(-21.27107, -1406.245, 29.63847),
		authorizedJobs = { 'cardealer' },
		objYaw = 90.0,
		locking = false,
		locked = true,
		pickable = false,
		distance = 2.0,
		size = 2
	},		
	-- Roldeur
	{
		objName = 'prop_com_gar_door_01',
		objCoords  = vector3(-21.04025, -1410.734, 30.51094),			
		textCoords = vector3(-21.04025, -1410.734, 30.51094),
		authorizedJobs = { 'cardealer' },
		locking = false,
		locked = true,
		pickable = false,
		distance = 15.0,
		size = 2
	},]]--

    -- Taco Doors
	--[[{
        objName = 'taco_drzwi',
        objYaw = -40.0,
        objCoords  = vector3(12.93, -1605.17, 29.37),
        textCoords = vector3(12.93, -1605.17, 29.37),
        authorizedJobs = {'taco', 'fib'},
		locking = false,
		locked = true,
		pickable = false,
		distance = 2.0,
		size = 2
    },

	{
        objName = 'taco_drzwi',
        objYaw = -40.0,
        objCoords  = vector3(19.27, -1599.36, 29.36),
        textCoords = vector3(19.27, -1599.36, 29.36),
        authorizedJobs = {'taco', 'fib'},
		locking = false,
		locked = true,
		pickable = false,
		distance = 2.0,
		size = 2
    },

	{
        objName = 'taco_drzwi',
        objYaw = 50.0,
        objCoords  = vector3(8.54, -1600.23, 29.39),
        textCoords = vector3(8.54, -1600.23, 29.39),
        authorizedJobs = {'taco', 'fib'},
		locking = false,
		locked = true,
		pickable = false,
		distance = 2.0,
		size = 2
    },

	{
        objName = 'taco_drzwi',
        objYaw = 50.0,
        objCoords  = vector3(20.41, -1604.92, 29.39),
        textCoords = vector3(19.9, -1604.7, 29.39),
        authorizedJobs = {'taco', 'fib'},
		locking = false,
		locked = true,
		pickable = false,
		distance = 2.0,
		size = 2
    },

	{
        objName = 'taco_drzwi',
        objYaw = -40.0,
        objCoords  = vector3(17.26, -1604.39, 29.38),
        textCoords = vector3(17.26, -1604.39, 29.38),
        authorizedJobs = {'taco', 'fib'},
		locking = false,
		locked = true,
		pickable = false,
		distance = 2.0,
		size = 2
	},]]--

	-- Court House
	{
		textCoords = vector3(103.83, -932.69, 29.84),
		authorizedJobs = { 'maor', 'police', 'emspolice', 'fib' },
		locking = false,
		locked = false,
		pickable = false,
		distance = 4,
		doors = {
			{
				objName = 'hei_prop_hei_bankdoor_new',
				objYaw = 70.0,
				objCoords  = vector3(103.02, -933.12, 29.84)
			},
	
			{
				objName = 'hei_prop_hei_bankdoor_new',
				objYaw = -108.0,
				objCoords  = vector3(104.03, -931.98, 29.84)
			}
		}
	},

	-- Unicorn Vanillia Doors

	-- Front
	{
		objName = 'prop_strip_door_01',
		objCoords = vector3(128.58, -1298.23, 30.23),
		textCoords = vector3(128.9, -1298.52, 30.23),
		authorizedJobs = { 'unicorn', 'fib' },
		locking = false,
		locked = false,
		pickable = false,
		distance = 2.0,
		size = 2
	},
	-- Back
	{
		objName = 'prop_magenta_door',
		objCoords = vector3(95.59, -1285.23, 30.27),
		textCoords = vector3(95.69, -1285.47, 29.27),
		authorizedJobs = { 'unicorn', 'fib' },
		locking = false,
		locked = false,
		pickable = false,
		distance = 2.0,
		size = 2
	},

	{
		objName = 'v_ilev_roc_door2',
		objYaw = -50.0,
		objCoords = vector3(99.63, -1293.29, 30.27),
		textCoords = vector3(99.56, -1292.71, 30.27),
		authorizedJobs = { 'vanilla' },
		locking = false,
		locked = false,
		pickable = false,
		distance = 1.5,
	},
	{
		objName = 'v_ilev_roc_door2',
		objYaw = 183.8,
		objCoords = vector3(954.68, -972.51, 40.50),
		textCoords = vector3(954.68, -972.51, 38.93),
		authorizedJobs = { 'mechanic' },
		locking = false,
		locked = true,
		pickable = false,
		distance = 2.0,
		size = 2
	},

	{
		objName = 'v_ilev_roc_door3',
		objYaw = 93.5,
		objCoords = vector3(948.48, -964.70, 40.50),
		textCoords = vector3(948.48, -964.70, 40.50),
		authorizedJobs = { 'mechanic' },
		locking = false,
		locked = true,
		pickable = false,
		distance = 2.0,
		size = 2
	},
	-- mechanic shop

	{
		objName = 'vesp_dopr',
		objYaw = -142.5,
		objCoords  = vector3(-1090.02, -841.02, 37.2),
		textCoords = vector3(-1090.02, -841.02, 37.2),
		authorizedJobs = { 'police', 'emspolice' },
		locking = false,
		locked = true,
		pickable = false,
		distance = 1.5,
	},
	-- Pillbox Medical Center
	{
		textCoords = vector3(348.56, -587.38, 44.28),
		authorizedJobs = { 'police', 'emspolice', 'ambulance', 'offambulance', 'fib'},
		locking = false,
		locked = true,
		pickable = false,
		distance = 2.0,
		size = 2,
		doors = {
			{
				objName = 'gabz_pillbox_doubledoor_l',
				objYaw = 250.0,
				objCoords  = vector3(349.134, -586.8831, 43.28402)
			},

			{
				objName = 'gabz_pillbox_doubledoor_r',
				objYaw = -110.0,
				objCoords  = vector3(348.6142, -588.1655, 43.28402)
			}
		}
	},

	{
		objName = 'gabz_pillbox_singledoor',
		objCoords = vector3(308.5733, -597.5756, 43.28403),
		textCoords = vector3(308.5733, -597.5756, 43.28403),
		authorizedJobs = { 'police', 'emspolice', 'ambulance', 'offambulance', 'fib'},
		locking = false,
		locked = true,
		pickable = false,
		distance = 2.0,
		size = 2
	},

	{
		objName = 'gabz_pillbox_singledoor',
		objCoords = vector3(340.1134, -586.6057, 44.28398),
		textCoords = vector3(340.1134, -586.6057, 44.28398),
		authorizedJobs = { 'police', 'emspolice', 'ambulance', 'offambulance', 'fib'},
		locking = false,
		locked = true,
		pickable = false,
		distance = 2.0,
		size = 2
	},

	{
		objName = 'gabz_pillbox_singledoor',
		objCoords = vector3(360.9867, -589.8188, 44.28403),
		textCoords = vector3(360.9867, -589.8188, 44.28403),
		authorizedJobs = { 'police', 'emspolice', 'ambulance', 'offambulance', 'fib'},
		locking = false,
		locked = true,
		pickable = false,
		distance = 2.0,
		size = 2
	},

	-- Pillbox Surgery #1
	{
		textCoords = vector3(313.22, -571.79, 44.29),
		authorizedJobs = { 'police', 'emspolice', 'ambulance', 'offambulance', 'fib'},
		locking = false,
		locked = true,
		pickable = false,
		distance = 2.0,
		size = 2,
		doors = {
			{
				objName = 'gabz_pillbox_doubledoor_l',
				objYaw = -20.0,
				objCoords  = vector3(312.7201, -571.6296, 43.28402)
			},

			{
				objName = 'gabz_pillbox_doubledoor_r',
				objYaw = -20.0,
				objCoords  = vector3(313.8816, -571.9534, 43.28402)
			}
		}
	},

	{
		textCoords = vector3(326.55, -579.42, 43.28),
		authorizedJobs = { 'police', 'emspolice', 'ambulance', 'offambulance' },
		locking = false,
		locked = true,
		pickable = false,
		distance = 2.0,
		size = 2,
		doors = {
			{
				objName = 'gabz_pillbox_doubledoor_l',
				objYaw = 250.0,
				objCoords  = vector3(326.54, -578.91, 43.28402)
			},

			{
				objName = 'gabz_pillbox_doubledoor_r',
				objYaw = 250.0,
				objCoords  = vector3(326.54, -580.04, 43.28402)
			}
		}
	},

	{
		textCoords = vector3(325.55, -589.02, 43.28),
		authorizedJobs = { 'police', 'emspolice', 'ambulance', 'offambulance' },
		locking = false,
		locked = true,
		pickable = false,
		distance = 2.0,
		size = 2,
		doors = {
			{
				objName = 'gabz_pillbox_doubledoor_l',
				objYaw = -20.0,
				objCoords  = vector3(324.91, -589.02, 43.28)
			},

			{
				objName = 'gabz_pillbox_doubledoor_r',
				objYaw = -20.0,
				objCoords  = vector3(326.16, -589.02, 43.28)
			}
		}
	},

	-- Pillbox Surgery #2

	{
		textCoords = vector3(319.07, -573.91, 44.29),
		authorizedJobs = { 'police', 'emspolice', 'ambulance', 'offambulance', 'fib'},
		locking = false,
		locked = false,
		pickable = false,
		distance = 2.0,
		size = 2,
		doors = {
			{
				objName = 'gabz_pillbox_doubledoor_l',
				objYaw = -20.0,
				objCoords  = vector3(318.4946, -573.7322, 43.28399)
			},

			{
				objName = 'gabz_pillbox_doubledoor_r',
				objYaw = -20.0,
				objCoords  = vector3(319.6151, -574.2166, 43.28405)
			}
		}
	},

	{
		textCoords = vector3(324.44, -575.64, 44.29),
		authorizedJobs = { 'police', 'emspolice', 'ambulance', 'offambulance', 'fib'},
		locking = false,
		locked = true,
		pickable = false,
		distance = 2.0,
		size = 2,
		doors = {
			{
				objName = 'gabz_pillbox_doubledoor_l',
				objYaw = -20.0,
				objCoords  = vector3(323.8446, -575.741, 43.284)
			},

			{
				objName = 'gabz_pillbox_doubledoor_r',
				objYaw = -20.0,
				objCoords  = vector3(325.0642, -576.1204, 43.284)
			}
		}
	},
-- Burgershot
	{
		objName = 'bs_cj_int_door_24',
		objYaw = 215.0,
		objCoords = vector3(-1200.28, -892.40, 14.90),
		textCoords = vector3(-1200.28, -892.40, 14.72),
		authorizedJobs = { 'burgershot' },
		locking = false,
		locked = true,
		pickable = false,
		distance = 2.0,
		size = 2
	},
	{
		objName = 'bs_cj_int_door_freezer',
		objYaw = 215.0,
		objCoords = vector3(-1202.77, -894.2, 14.90),
		textCoords = vector3(-1202.77, -894.2, 14.90),
		authorizedJobs = { 'burgershot' },
		locking = false,
		locked = true,
		pickable = false,
		distance = 2.0,
		size = 2
	},

	{
		objName = 'bs_cj_int_door_24',
		objYaw = 125.0,
		objCoords = vector3(-1195.67, -897.27, 14.90),
		textCoords = vector3(-1195.67, -897.27, 14.72),
		authorizedJobs = { 'burgershot' },
		locking = false,
		locked = true,
		pickable = false,
		distance = 2.0,
		size = 2
	},
	
	{
		objName = 'p_bs_map_door_01_s',
		objYaw = 125.0,
		objCoords = vector3(-1178.98, -892.01, 14.90),
		textCoords = vector3(-1178.98, -892.01, 14.90),
		authorizedJobs = { 'burgershot' },
		locking = false,
		locked = true,
		pickable = false,
		distance = 2.0,
		size = 2
	},
--bcso
	{
		objName = 'v_ilev_shrfdoor',
		objYaw = 31.0,
		objCoords = vector3(1855.12, 3683.59, 35.27),
		textCoords = vector3(1855.12, 3683.59, 35.27),
		authorizedJobs = { 'police', 'emspolice' },
		locking = false,
		locked = false,
		pickable = false,
		distance = 2.0,
		size = 2
	},

	{
		objName = 'v_ilev_cd_entrydoor',
		objYaw = -60.0,
		objCoords = vector3(1844.55, 3693.55, 35.27),
		textCoords = vector3(1844.55, 3693.55, 35.27),
		authorizedJobs = { 'police', 'emspolice' },
		locking = false,
		locked = true,
		pickable = false,
		distance = 2.0,
		size = 2
	},

	{
		objName = 'v_ilev_rc_door2',
		objYaw = -150.0,
		objCoords = vector3(1856.68, 3690.02, 35.27),
		textCoords = vector3(1856.68, 3690.02, 35.27),
		authorizedJobs = { 'police', 'emspolice' },
		locking = false,
		locked = true,
		pickable = false,
		distance = 2.0,
		size = 2
	},

	{
		textCoords = vector3(1850.65, 3682.99, 35.25),
		authorizedJobs = { 'police', 'emspolice' },
		locking = false,
		locked = true,
		pickable = false,
		distance = 2.5,
		doors = {
			{
				objName = 'v_ilev_rc_door2',
				objYaw = 120.5,
				objCoords = vector3(1850.97, 3682.37, 33.96)
			},

			{
				objName = 'v_ilev_rc_door2',
				objYaw = -60.0,
				objCoords = vector3(1850.29, 3683.59, 33.96)
			}
		}
	},

	{
		objName = 'v_ilev_ph_cellgate',
		objYaw = -60.0,
		objCoords = vector3(1858.67, 3695.62, 31.26),
		textCoords = vector3(1858.67, 3695.62, 30.26),
		authorizedJobs = { 'police', 'emspolice' },
		locking = false,
		locked = true,
		pickable = false,
		distance = 2.0,
		size = 2
	},

	{
		objName = 'v_ilev_ph_cellgate',
		objYaw = -60.0,
		objCoords = vector3(1860.57, 3692.32, 31.26),
		textCoords = vector3(1860.57, 3692.32, 30.26),
		authorizedJobs = { 'police', 'emspolice' },
		locking = false,
		locked = true,
		pickable = false,
		distance = 2.0,
		size = 2
	},

	{
		objName = 'v_ilev_ph_cellgate',
		objYaw = -60.0,
		objCoords = vector3(1862.41, 3689.0, 31.26),
		textCoords = vector3(1862.41, 3689.0, 30.26),
		authorizedJobs = { 'police', 'emspolice' },
		locking = false,
		locked = true,
		pickable = false,
		distance = 2.0,
		size = 2
	},

	{
		objName = 'v_ilev_ph_cellgate',
		objYaw = -60.0,
		objCoords = vector3(1859.31, 3687.33, 31.26),
		textCoords = vector3(1859.31, 3687.33, 32.26),
		authorizedJobs = { 'police', 'emspolice' },
		locking = false,
		locked = true,
		pickable = false,
		distance = 2.0,
		size = 2
	},

	{
		textCoords = vector3(1850.33, 3682.72, 31.25),
		authorizedJobs = { 'police', 'emspolice' },
		locking = false,
		locked = true,
		pickable = false,
		distance = 2.5,
		doors = {
			{
				objName = 'v_ilev_rc_door2',
				objYaw = 120.5,
				objCoords = vector3(1850.97, 3682.37, 31.96)
			},

			{
				objName = 'v_ilev_rc_door2',
				objYaw = -60.0,
				objCoords = vector3(1850.29, 3683.59, 31.96)
			}
		}
	},

	{
		textCoords = vector3(1848.27, 3690.57, 35.25),
		authorizedJobs = { 'police', 'emspolice' },
		locking = false,
		locked = true,
		pickable = false,
		distance = 2.5,
		doors = {
			{
				objName = 'v_ilev_rc_door2',
				objYaw = 208.4,
				objCoords = vector3(1848.97, 3690.96, 33.96)
			},

			{
				objName = 'v_ilev_rc_door2',
				objYaw = 30.0,
				objCoords = vector3(1847.58, 3690.17, 33.96)
			}
		}
	},

	{
		textCoords = vector3(1847.53, 3683.29, 35.25),
		authorizedJobs = { 'police', 'emspolice' },
		locking = false,
		locked = true,
		pickable = false,
		distance = 2.5,
		doors = {
			{
				objName = 'v_ilev_rc_door2',
				objYaw = 30.0,
				objCoords = vector3(1847.05, 3682.92, 35.25)
			},

			{
				objName = 'v_ilev_rc_door2',
				objYaw = -149.8,
				objCoords = vector3(1848.09, 3683.52, 35.25)
			}
		}
	},

	{
		textCoords = vector3(1847.53, 3683.29, 31.25),
		authorizedJobs = { 'police', 'emspolice' },
		locking = false,
		locked = true,
		pickable = false,
		distance = 2.5,
		doors = {
			{
				objName = 'v_ilev_rc_door2',
				objYaw = 30.0,
				objCoords = vector3(1847.05, 3682.92, 31.25)
			},

			{
				objName = 'v_ilev_rc_door2',
				objYaw = -149.8,
				objCoords = vector3(1848.09, 3683.52, 31.25)
			}
		}
	},
}



--HaReL#5843 Extra Leaks

--[[{
	objName = 'v_ilev_rc_door2',
	objCoords  = {x = 1850.79, y = 3682.70, z = 34.27}, ----------------------
	textCoords = {x = 1850.79, y = 3682.70, z = 34.27},
	authorizedJobs = { 'police', 'emspolice' },
	locked = true,
	distance = 2.5
},

{
	objName = 'v_ilev_rc_door2',
	objCoords  = {x = 1847.83, y = 3690.33, z = 34.27}, ----------------------
	textCoords = {x = 1847.83, y = 3690.33, z = 34.27},
	authorizedJobs = { 'police', 'emspolice' },
	locked = true,
	distance = 2.5
},

{
	objName = 'v_ilev_cd_entrydoor',
	objCoords  = {x = 1844.77, y = 3693.45, z = 34.27}, ----------------------
	textCoords = {x = 1844.77, y = 3693.45, z = 34.27},
	authorizedJobs = { 'police', 'emspolice' },
	locked = true,
	distance = 2.5
},

{
	objName = 'v_ilev_rc_door2',
	objCoords  = {x = 1848.73, y = 3690.99, z = 34.27}, ----------------------
	textCoords = {x = 1848.73, y = 3690.99, z = 34.27},
	authorizedJobs = { 'police', 'emspolice' },
	locked = true,
	distance = 2.5
},

{
	objName = 'v_ilev_rc_door2',
	objCoords  = {x = 1850.31, y = 3683.39, z = 34.27}, ----------------------
	textCoords = {x = 1850.31, y = 3683.39, z = 34.27},
	authorizedJobs = { 'police', 'emspolice' },
	locked = true,
	distance = 2.5
},

{
	objName = 'v_ilev_ph_cellgate',
	objCoords  = {x = 1858.67, y = 3695.62, z = 30.26},
	textCoords = {x = 1858.67, y = 3695.62, z = 30.26},
	authorizedJobs = { 'police', 'emspolice' },
	locked = true
},

{
	objName = 'v_ilev_ph_cellgate',
	objCoords  = {x = 1860.57, y = 3692.32, z = 30.26},
	textCoords = {x = 1860.57, y = 3692.32, z = 30.26},
	authorizedJobs = { 'police', 'emspolice' },
	locked = true
},

{
	objName = 'v_ilev_ph_cellgate',
	objCoords  = {x = 1862.41, y = 3689.0, z = 30.26},
	textCoords = {x = 1862.41, y = 3689.0, z = 30.26},
	authorizedJobs = { 'police', 'emspolice' },
	locked = true
},

{
	objName = 'v_ilev_ph_cellgate',
	objCoords  = {x = 1859.31, y = 3687.33, z = 30.26},
	textCoords = {x = 1859.31, y = 3687.33, z = 30.26},
	authorizedJobs = { 'police', 'emspolice' },
	locked = true
},]]--